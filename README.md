...
- Optional 1Password save via `op` CLI using its Crypto Wallet JSON template over stdin, so the recovery phrase does not appear in process arguments.
...
- The item JSON template, including the recovery phrase, is passed to `op` through `/dev/stdin` rather than as command-line assignments.
- Fields set:
  - `Recovery phrase` (concealed).
  - Crypto Wallet's built-in `wallet address` field for EVM.
  - Additional BTC/SOL address fields in the Wallet section.
